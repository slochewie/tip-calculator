...
export function AccountSwitcherSubmenu({ currentUserId, consoleBaseURL }: { currentUserId: string; consoleBaseURL: string }) {
  ...
  async function switchAccount(deviceSession: DeviceSession) {
    ...
    const { error } = await authClient.multiSession.setActive({ sessionToken: deviceSession.session.token })
    if (error) {
      ...
      <DropdownMenuSubContent className="w-72">
        {isLoading ? (
          ...
          deviceSessions.map((deviceSession) => {
            ...
            return (
              <DropdownMenuItem key={deviceSession.session.token} disabled={isCurrent || Boolean(switchingToken)} onClick={() => void switchAccount(deviceSession)} className="py-2">
                <Avatar className="size-8">
                  {deviceSession.user.image ? <AvatarImage src={deviceSession.user.image} alt="" /> : null}
                  <AvatarFallback>{getInitials(label)}</AvatarFallback>
                </Avatar>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium">{label}</p>
                  <p className="truncate text-xs text-muted-foreground">{deviceSession.user.email}</p>
                </div>
                {isCurrent ? <CheckIcon className="ml-auto" /> : null}
                {isSwitching ? <span className="ml-auto text-xs text-muted-foreground">Switching…</span> : null}
              </DropdownMenuItem>
            )
          })
        )}
