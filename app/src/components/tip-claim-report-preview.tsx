...
export type TipClaimPreviewRole = "bartender" | "manager" | "barback" | "door";
export type TipClaimPreviewWeights = Record<TipClaimPreviewRole, number>;

export type TipClaimPreviewStaff = {
  userId: string;
  name: string;
  email: string;
  role: TipClaimPreviewRole;
};
...
type TipClaimReportPreviewProps = {
  ...
  weights: TipClaimPreviewWeights;
  savePending?: boolean;
  onSave: (weights: TipClaimPreviewWeights) => void | Promise<void>;
};
...
const ROLE_ORDER: TipClaimPreviewRole[] = [
  "bartender",
  "manager",
  "barback",
  "door",
...
const ROLE_LABELS: Record<TipClaimPreviewRole, string> = {
  bartender: "Bartender",
  manager: "Manager",
  barback: "Barback",
  door: "Door",
...
const ROLE_COLORS: Record<TipClaimPreviewRole, string> = {
...
function allocatePreview(
  ...
  const weightedStaff = ROLE_ORDER.flatMap((role) =>
    staff
      .filter((person) => person.role === role)
      .map((person) => ({
        ...person,
        weight: Math.max(0, weights[role]),
      })),
  ).filter((person) => person.weight > 0);

  const totalWeight = weightedStaff.reduce((sum, person) => sum + person.weight, 0);

  if (totalCents <= 0 || totalWeight <= 0 || weightedStaff.length === 0) {
    return staff.map((person) => ({ ...person, cents: 0 }));
  }

  const provisional = weightedStaff.map((person, index) => {
  ...
  const remaining =
    totalCents - provisional.reduce((sum, person) => sum + person.cents, 0);
  ...
  const allocationByUserId = new Map(
    provisional.map((person) => [person.userId, person.cents]),
  );
  ...
  return ROLE_ORDER.flatMap((role) =>
    staff
      ...
      .map((person) => ({
        ...person,
        cents: allocationByUserId.get(person.userId) ?? 0,
      })),
  );
}
...
export function TipClaimReportPreview({
  ...
  const [previewWeights, setPreviewWeights] = useState<TipClaimPreviewWeights>(weights);
  const [initialWeights, setInitialWeights] = useState<TipClaimPreviewWeights>(weights);
  ...
  const hasAllocation = requiredClaimCents === 0 || allocatedCents === requiredClaimCents;
