...
export function TipWeightPresetConfigurator({
  ...
  const [expandedRoles, setExpandedRoles] = useState<Record<TipClaimRoleKey, boolean>>({
    manager: false,
    bartender: true,
    barback: true,
    door: true,
    ...
    <main className="mx-auto flex w-full max-w-6xl flex-col gap-5 p-4 md:p-6 lg:p-8">
      <div className="flex flex-col gap-2">
        <div className="flex flex-wrap items-center gap-2">
          <h1 className="font-heading text-3xl font-semibold tracking-tight">Weight Presets</h1>
          ...
          <Card>
            ...
            <CardContent className="flex flex-col gap-5">
              <Field>
                ...
                />
                <FieldDescription>Use a name that makes the staffing state easy to recognize.</FieldDescription>
              </Field>
          ...
          <CardContent className="flex flex-col gap-4">
            ...
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
              {roleData.map((item) => {
                ...
                return (
                  <div key={item.role} className="flex flex-col rounded-lg border">
                    <button
                      type="button"
                    ...
                    >
                      <div className="flex min-w-0 items-center gap-2">
                        <span className="size-2.5 shrink-0 rounded-sm" style={{ backgroundColor: item.fill }} />
                        <span className="truncate font-medium">{TIP_CLAIM_ROLE_LABELS[item.role]}</span>
                      </div>
                      <div className="flex items-start gap-2">
                        <div className="text-right tabular-nums">
                          <div className="font-semibold">{item.percentage.toLocaleString("en-US", { maximumFractionDigits: 1 })}%</div>
                          <div className="text-xs text-muted-foreground">{item.staff} × {item.weight}</div>
                        </div>
                        <ChevronDownIcon className={`mt-0.5 size-4 shrink-0 text-muted-foreground transition-transform ${expanded ? "rotate-180" : ""}`} />
                      </div>
                    </button>

                    {expanded ? (
                      <div id={`preset-role-${item.role}`} className="flex flex-col gap-2 border-t p-3">
