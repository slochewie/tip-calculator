...
type AvailableTipClaimOrganization = {
  organizationId: string;
  organizationName: string;
};

type AvailableTipClaimOrganizationsResponse = {
  organizations?: AvailableTipClaimOrganization[];
};
...
function getTipClaimResourceUrl(request: Request) {
  ...
  if (
    host === "tip-calculator.niteowl.dev" ||
    host === "tip-calculator.mccarthysirishpub.com"
  ) {
    return `https://${host}`;
  }
...
function rejectJwt(reason: string, details: Record<string, unknown> = {}) {
  console.warn("[tip-claim-auth] JWT rejected", {
    reason,
    ...details,
  });
  return null;
...
async function verifyJwt(request: Request): Promise<VerifiedTipClaimToken | null> {
  ...
  const tipClaimResourceUrl = getTipClaimResourceUrl(request);
  const browserAudience = audienceIncludes(payload.aud, authBaseUrl);
  const resourceAudience = audienceIncludes(payload.aud, tipClaimResourceUrl);
  ...
  if (!browserAudience && !resourceAudience) {
    return rejectJwt("audience_mismatch", {
      ...
      expectedAudiences: [authBaseUrl, tipClaimResourceUrl],
    });
  }
...
export async function hasTipClaimScope(request: Request, requiredScope: string) {
  const verified = await verifyJwt(request);
...
export async function getAvailableTipClaimOrganizations(request: Request) {
  ...
  const url = new URL(
    `${getAuthBaseUrl(request)}/api/auth/tip-claim/available/internal`,
  );
  ...
  const result = (await response.json()) as AvailableTipClaimOrganizationsResponse;
  return result.organizations ?? [];
}

export async function getTipClaimOrganizationAccess(
  request: Request,
  organizationId: string,
) {
  const userId = await getAuthenticatedUserId(request);
  ...
  const url = new URL(`${getAuthBaseUrl(request)}/api/auth/tip-claim/access/internal`);
