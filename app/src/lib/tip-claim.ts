...
export async function listTipClaimShifts(organizationId: string) {
  ...
  if (!response.ok) throw new Error(typeof result.error === "string" ? result.error : "Unable to load tip claim reports.");
  return Array.isArray(result.shifts) ? result.shifts : [];
}

export async function deleteTipClaimShift(organizationId: string, shiftId: string) {
  const url = new URL("/api/auth/tip-claim/shifts", authBaseURL);
  const response = await fetch(url, { method: "DELETE", credentials: "include", headers: { "content-type": "application/json" }, body: JSON.stringify({ organizationId, shiftId }) });
  const result = (await response.json()) as TipClaimDeleteResponse;
  if (!response.ok) throw new Error(typeof result.error === "string" ? result.error : "Unable to delete shift.");
  if (result.shiftId !== shiftId) throw new Error("Deleted shift response did not match the requested shift.");
  return { shiftId: result.shiftId };
}
...
export async function listTipClaimEmployees(organizationId: string) {
  ...
  if (!response.ok) throw new Error(typeof result.error === "string" ? result.error : "Unable to load employees.");
...
export async function getTipClaimAccess(organizationId: string) {
  ...
  if (!response.ok) throw new Error(typeof result.error === "string" ? result.error : "Unable to check Tip Calculator access.");
  return result.allowed === true;
}

async function updateAssignmentFlag(path: string, organizationId: string, userId: string, enabled: boolean, fallbackError: string) {
  const url = new URL(path, authBaseURL);
  const response = await fetch(url, { method: "PATCH", credentials: "include", headers: { "content-type": "application/json" }, body: JSON.stringify({ organizationId, userId, enabled }) });
  const result = (await response.json()) as TipClaimAssignmentUpdateResponse;
  if (!response.ok) throw new Error(typeof result.error === "string" ? result.error : fallbackError);
  if (!result.assignment) throw new Error("Assignment update completed without an assignment.");
  return result.assignment;
}

export function updateTipClaimAccess(organizationId: string, userId: string, enabled: boolean) {
  return updateAssignmentFlag("/api/auth/tip-claim/access", organizationId, userId, enabled, "Unable to update Tip Calculator access.");
}

export function updateTipClaimManager(organizationId: string, userId: string, enabled: boolean) {
...
export async function updateTipClaimAssignment(organizationId: string, userId: string, role: TipClaimRole, enabled: boolean) {
  ...
  const result = (await response.json()) as TipClaimAssignmentUpdateResponse;
  if (!response.ok) throw new Error(typeof result.error === "string" ? result.error : "Unable to update employee assignment.");
  if (!result.assignment) throw new Error("Assignment update completed without an assignment.");
  return result.assignment;
}
