...
export const Route = createFileRoute("/api/tip-claim/tip-pool-shifts")({
  server: {
    handlers: {
      GET: async ({ request }: { request: Request }) => {
        ...
        try {
          const result = await fetchTipClaimInternalJson<TipPoolShiftsResponse>(
            request,
            "/api/auth/tip-claim/tip-pool-shifts/internal",
            auth.userId,
            { organizationId },
          );

          if (!result) {
            return jsonError("Unable to load Tip Calculator tip pool reports", 502);
          }
          ...
          return jsonError(
            error instanceof Error
              ? error.message
              : "Unable to load Tip Calculator tip pool reports",
            502,
          );
        }
      },
      ...
      PATCH: async ({ request }: { request: Request }) => {
        ...
        try {
          ...
          return jsonError(
            error instanceof Error
              ? error.message
              : "Unable to correct tip pool report",
            502,
          );
        }
      },
      ...
      DELETE: async ({ request }: { request: Request }) => {
        ...
        try {
          ...
          return jsonError(
            error instanceof Error
              ? error.message
              : "Unable to delete tip pool report",
            502,
          );
        }
      },
